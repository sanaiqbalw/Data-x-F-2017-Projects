...
pickle_root = os.path.join(data_root, 'pickle_data_combined')
sample_list_root = os.path.join(data_root, 'sample_lists')

# where to save the model
model_root = os.path.join(data_root, 'models')
model_file = os.path.join(model_root, 'standard_OT500.ckpt')

# load in dataframes containing all information about samples
train_csv = os.path.join(sample_list_root, 'train_sample_list_2.csv')
valid_csv = os.path.join(sample_list_root, 'valid_sample_list_2.csv')
...
train_os_filenames = list(df_train_os['sample_path'].values)
train_filenames = list(df_train['sample_path'].values)
valid_filenames = list(df_valid['sample_path'].values)

train_pos_filenames = list(df_train[df_train['worms'] > 1]['sample_path'].values)
train_neg_filenames = list(df_train[df_train['worms'] <= 1]['sample_path'].values)
valid_pos_filenames = list(df_valid[df_valid['worms'] > 1]['sample_path'].values)
...
def make_dataset_strat(pos_filenames, neg_filenames, name='datasets_strat', batch_size=8, repeat=False):
    """Create TensorFlow Dataset objects from filenames"""
    with tf.variable_scope(name) as scope:
        # positive dataset
        ...
        neg_dataset = neg_dataset.shuffle(buffer_size=1000)
...
#     train_dataset = make_dataset_strat(
#         train_pos_filenames, train_neg_filenames, name='train_dataset', batch_size=batch_size, repeat=True)
...
def model(name, conv_dropout_keep_prob=1.0, fc_dropout_keep_prob=1.0, reuse=None):
    ...
    with tf.variable_scope(name, reuse=reuse) as scope:
        # input format
        input_shape = [80, 80, 40]  # can't actually get rid of this 
        num_labels = 2
        ...
        # regularizers
        reg_conv = tf.contrib.layers.l2_regularizer(scale=1e-3)
        reg_fc = tf.contrib.layers.l2_regularizer(scale=1e-3)
...
#         conv_3 = maxpool_layer(conv_3, name='maxpool_3')
        ...
        conv_4 = conv_relu_layer(conv_3,
                                 ...
                                 dropout_keep_prob=conv_dropout_keep_prob,
                                 name='conv_4')
        conv_4 = maxpool_layer(conv_4, name='maxpool_4')
...
with tf.name_scope('loss') as scope:
    ...
    if weight_samples == 0:
        ...
        loss = tf.reduce_mean(tf.nn.weighted_cross_entropy_with_logits(
            targets=y_batch, logits=logits_drop, pos_weight=7.0) + tf.add_n(l2_losses))
