...
sample_list_root = os.path.join(data_root, 'sample_lists_temp')
sample_dfs = [pd.read_csv(os.path.join(sample_list_root, f)) for f in os.listdir(sample_list_root)]
df = pd.concat(sample_dfs, axis=0)
pre_sample_count = df.shape[0]

# drop bme score
df = df.drop(['bme'], axis=1)

# drop any rows with files that don't exist
df = df[df['sample_path'].apply(lambda p: os.path.exists(p))]
post_sample_count = df.shape[0]

print('Count before processing: {}'.format(pre_sample_count))
print('Count after processing: {}'.format(post_sample_count))

# fill in age and bmi with median
df['AGE'] = df['AGE'].fillna(df['AGE'].median())
df['BMI'] = df['BMI'].fillna(df['BMI'].median())

# set null genders to 0
...
df_train.to_csv(os.path.join(sample_list_root, 'train_sample_list_2.csv'), index=False)
df_valid.to_csv(os.path.join(sample_list_root, 'valid_sample_list_2.csv'), index=False)
# df.to_csv(os.path.join(sample_list_root, 'full_sample_list_1.csv'), index=False)
